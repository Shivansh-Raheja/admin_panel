...
const Coupons = () => {
  ...
  const fetchCoupons = async () => {
    try {
      const response = await axios.get("http://localhost/admin_api/coupon.php");
  ...
  const handleSaveCoupon = async () => {
    try {
      if (formData.id) {
        await axios.put("http://localhost/admin_api/coupon.php", formData);
        Swal.fire("Updated!", "Coupon updated successfully.", "success");
      } else {
        await axios.post("http://localhost/admin_api/coupon.php", formData);
  ...
  const handleDeleteCoupon = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios.delete("http://localhost/admin_api/coupon.php", {
          data: { id },
        });
        fetchCoupons();
