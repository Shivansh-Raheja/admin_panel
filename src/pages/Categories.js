...
const Categories = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get("http://localhost/admin_api/categories.php");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        await axios.put("http://localhost/admin_api/categories.php", formData);
        Swal.fire("Updated!", "Category updated successfully.", "success");
      } else {
        await axios.post("http://localhost/admin_api/categories.php", formData);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete("http://localhost/admin_api/categories.php", { data: { id } });
        ...
        <tbody>
          {categories.map((category) => (
            <tr key={category.id}>
              ...
              <td>
                <img src={`http://localhost/admin_api/${category.image}`} alt="Category" className="category-img" />
              </td>
              <td>
