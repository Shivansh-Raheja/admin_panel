...
const Banners = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await axios.get("https://api.wonderplastpanel.in/admin_api/banners.php");
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://api.wonderplastpanel.in/admin_api/banners.php`, { data: { id } });
          fetchBanners();
        ...
        <tbody>
          {banners.map((banner) => (
            <tr key={banner.id}>
              ...
              <td>{banner.title}</td>
              <td>
                {banner.image && <img src={`https://api.wonderplastpanel.in/admin_api/${banner.image}`} alt="Banner" className="banner-img" />}
              </td>
