...
const Banners = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await axios.get("https://api.magnumwonderplast.com/admin_api/banners.php");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        ...
        await axios.put("https://api.magnumwonderplast.com/admin_api/banners.php", formDataToSend);
        Swal.fire("Updated!", "Banner updated successfully.", "success");
      } else {
        await axios.post("https://api.magnumwonderplast.com/admin_api/banners.php", formDataToSend);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://api.magnumwonderplast.com/admin_api/banners.php`, { data: { id } });
          fetchBanners();
        ...
        <tbody>
          {banners.map((banner) => (
            <tr key={banner.id}>
              ...
              <td>{banner.title}</td>
              <td>
                {banner.image && <img src={`https://api.magnumwonderplast.com/admin_api/${banner.image}`} alt="Banner" className="banner-img" />}
              </td>
