...
const Banners = () => {
  ...
  const fetchBanners = async () => {
    try {
      const response = await axios.get("http://api.magnumwonderplast.com/admin_api/banners.php");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        ...
        await axios.put("http://api.magnumwonderplast.com/admin_api/banners.php", formDataToSend);
        Swal.fire("Updated!", "Banner updated successfully.", "success");
      } else {
        await axios.post("http://api.magnumwonderplast.com/admin_api/banners.php", formDataToSend);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://api.magnumwonderplast.com/admin_api/banners.php`, { data: { id } });
          fetchBanners();
        ...
        <tbody>
          {banners.map((banner) => (
            <tr key={banner.id}>
              ...
              <td>{banner.title}</td>
              <td>
                {banner.image && <img src={`http://api.magnumwonderplast.com/admin_api/${banner.image}`} alt="Banner" className="banner-img" />}
              </td>
