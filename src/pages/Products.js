...
const Products = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get("http://localhost/admin_api/products.php");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost/admin_api/products.php", productData, {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete("http://localhost/admin_api/products.php", { data: { id } });
            ...
            <tr key={product.id}>
              ...
              <td>
                {product.images && product.images.length > 0 && (
                  <img
                    src={`http://localhost/admin_api/${product.images[0]}`}
                    alt="Product"
