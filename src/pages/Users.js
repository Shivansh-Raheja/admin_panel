...
const Users = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("https://api.wonderplastpanel.in/admin_api/users.php");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("https://api.wonderplastpanel.in/admin_api/users.php", formData);
      if (response.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.delete(`https://api.wonderplastpanel.in/admin_api/users.php?id=${id}`);
